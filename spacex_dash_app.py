...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        data = spacex_df.loc[spacex_df['Launch Site'] == entered_site].groupby('class')[['class']].count().rename({'class':'class_count'},axis = 1).reset_index()

        
        fig = px.pie(data, values = 'class_count', names = 'class' , title = f'Total Success Launches for site "{entered_site}"')
        return fig
